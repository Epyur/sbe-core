...
### 2026-09-04 — фикс `StoreManager.apply()`: обновление ЦУП сверяло хэш со старым реестром

- Живая жалоба пользователя («опять ошибка несовпадения сумм» при обновлении
  плагина через ЦУП, повторявшаяся несколько раз за сессию) — `apply()`
  (`src/store-manager.ts`, вызывается и `install()`, и `update()`) брал
  `entry.hashes` из `this.registry` — реестра, загруженного в память при
  старте Obsidian или последнем открытии магазина, — а свежий `registry.json`
  подтягивался только ПОСЛЕ установки (`this.refresh()` в конце). Если хэши
  на сервере обновили (новый релиз, `registry_file_overrides`) уже ПОСЛЕ
  того, как ЦУП загрузил реестр в память, но ДО того, как пользователь нажал
  «Обновить» без предварительного переоткрытия магазина — `installPlugin`
  скачивал уже НОВЫЙ файл, а сверял его со СТАРЫМ хэшем из памяти →
  «Контрольная сумма не совпадает» на полностью корректном файле. Перезапуск
  Obsidian «чинил» проблему только потому, что заново грузил реестр с нуля —
  это маскировало гонку, а не признак настоящей причины (сервер всё это
  время был консистентен, проверено напрямую — hash `registry.json`
  всегда совпадал с реально отдаваемыми файлами).
- Фикс: `apply()` теперь зовёт `await this.refresh()` в самом начале, до
  `findEntry(id)`, — реестр в памяти актуализируется непосредственно перед
  использованием его хэшей, а не только после установки.
- `sbe-core` без собственной версии — единственный потребитель `store-
  manager.ts` (`sbe-apstore`, подтверждено `grep` по всем плагинам) пересобран
  и бампнут, см. его `AGENTS.md`.

### 2026-09-02 — фикс `isNewer()`: суффикс `b` ложно триггерил «доступно обновление»
- `isNewer(remote, local)` в `src/registry.ts` сравнивал версии через
  `local.split('.').map(Number)` — сегмент с суффиксом `b` (см. правило про
  версии на ветке `backend` в корневом `AGENTS.md`, например `"5b"` в
  `0.1.5b`) давал `Number("5b") = NaN`, а `NaN || 0` в цикле сравнения тихо
  обнулял сегмент. Локальная версия с суффиксом выглядела СТАРШЕ, чем есть —
  ЦУП ложно предлагал «обновление» до версии на `main`, которая на самом
  деле старше уже установленной (обнаружено на `sbe-llm` 0.1.5b и
  `sbe-photobank` 0.1.20b).
- Фикс: новая `parseVersionSegment()` — `parseInt(seg, 10) || 0` — читает
  числовой префикс, отбрасывая буквенный хвост, вместо `Number()`.
- `sbe-core` без собственной версии — потребитель (`sbe-apstore`, единственная
  реализация `isNewer`) пересобран и бампнут (v0.3.15), см. его `AGENTS.md`.

### 2026-09-02 — SbeLlmApi.getStatus() стал асинхронным (перенос ключа на сервер)
- **Единственное breaking-изменение** в `SbeLlmApi` (аддитивные изменения обычно не
  требуют пересборки потребителей — здесь другое: `sbe-llm` перенёс хранение ключа
  провайдера ИИ из `app.secretStorage` на сервер (новый `llm-service`, ключ зашифрован
  и привязан к email пользователя) — статус теперь живой запрос, не локальная проверка.
  `getStatus(): { configured, apiUrl }` → `getStatus(): Promise<{ configured, apiUrl }>`.
- Пересобран/бампнут только реальный потребитель метода — `sbe-photobank`
  (`ai-describe.service.ts`, 3 места добавлен `await`). Остальные потребители
  `getService('sbe-llm')` (если появятся) поправить аналогично при следующей правке.
- См. `sbe-llm/AGENTS.md` и `sbe-llm/llm-service/AGENTS.md` (архитектура хранения ключа).
